...
exports.getSessionsByTutor = async (req, res) => {
    try {
        await syncAttendance(req.params.tutorId, 'Host');
...
// END SESSION (Tutor ends the session for everyone)
exports.endSession = async (req, res) => {
    try {
        ...
        if (session.status === 'cancelled') {
            return res.status(400).json({ success: false, message: 'Cannot end a cancelled session' });
        }
        ...
        for (const booking of bookings) {
            if (booking.status === 'completed') continue; // Already processed

            // If not cancelled, they participated (as per user request)
        ...
        res.status(200).json({
            success: true,
            message: 'Session ended successfully. Attendance recorded for tutor and all students.',
            data: session
        });
    } catch (error) {
        res.status(400).json({
