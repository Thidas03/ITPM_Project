...
exports.getAttendanceReports = async (req, res) => {
    try {
        ...
        // Sync attendance for all students before generating report
        for (const user of users) {
            await syncAttendance(user._id, 'Student');
        }
        ...
        const attendedTotal = users.reduce((acc, u) => acc + (u.attendedSessions || 0), 0);
        const missedTotal = users.reduce((acc, u) => acc + (u.missedSessions || 0), 0);

        // 2. Real Trends Calculation
        ...
        const monthlyStats = await Booking.aggregate([
            ...
            },
            {
                $group: {
                    ...
                    attended: {
                        $sum: { $cond: [{ $eq: ["$status", "completed"] }, 1, 1] } 
                    },
                    missed: {
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            trends.push({
                month: months[m-1],
                attended: monthData ? monthData.attended : 0,
                missed: monthData ? monthData.missed : 0,
                cancelled: monthData ? monthData.cancelled : 0
            });
        }

        // 3. Misuse Behavior (Refined for cancellation focus)
        const misuseBehavior = users.filter(u => u.cancellations > 3 || u.misconductCount > 0)
            .map(u => ({
                _id: u._id,
                firstName: u.firstName,
                lastName: u.lastName,
        ...
        const allStudentAttendance = users.map(u => {
            const total = (u.attendedSessions || 0) + (u.missedSessions || 0);
            return {
                _id: u._id,
                name: `${u.firstName} ${u.lastName}`,
                email: u.email,
                attended: u.attendedSessions || 0,
                missed: u.missedSessions || 0,
