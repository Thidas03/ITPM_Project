...
const SessionRoom = () => {
    ...
    const handleEndSession = async () => {
        if (!window.confirm('Are you sure you want to end this session? This will record attendance for everyone and close the session.')) return;
        try {
                ...
                <div className="bg-gray-800 rounded-3xl border border-gray-700 overflow-hidden shadow-2xl">
                    ...
                    <div className="p-6 md:p-8 space-y-6">
                        ...
                        <div className="pt-5 border-t border-gray-700">
                            ...
                            {(user?.role === 'Host' || user?.role === 'Tutor') && (
                                <>
                                    {session.status === 'available' || session.status === 'booked' ? (
                                        ...
                                        <div className="p-6 bg-teal-500/10 border border-teal-500/30 rounded-2xl text-center">
                                            <p className="text-teal-400 font-black text-xl flex items-center justify-center gap-3">
                                                ✅ Session Completed
                                            </p>
                                            <p className="text-gray-400 text-sm mt-2">Attendance and reports have been finalized.</p>
                                        </div>
                                    ) : (
                                        <div className="p-6 bg-red-500/10 border border-red-500/30 rounded-2xl text-center">
                                            <p className="text-red-400 font-black text-xl flex items-center justify-center gap-3">
                                                ❌ Session Cancelled
                                            </p>
                                        </div>
                                    )}
                                ...
                                <>
                                    {session.status === 'available' || session.status === 'booked' ? (
                                            ...
                                            {!isJoined ? (
                                                ...
                                                <div className="space-y-4">
                                                    ...
                                                    <p className="text-center text-xs text-gray-500 italic">
                                                        * Attendance is recorded when you click "Leave" or when the Tutor ends the session.
                                                    </p>
                                                </div>
                                            )}
                                        </>
                                        ...
                                        <div className="p-6 bg-teal-500/10 border border-teal-500/30 rounded-2xl text-center">
                                            ...
                                            <p className="text-gray-400 text-sm mt-2">Check your history for attendance status.</p>
                                            <button 
                                                onClick={() => navigate('/my-sessions')}
                                                className="mt-4 px-6 py-2 bg-gray-800 hover:bg-gray-700 text-white rounded-xl text-sm font-bold transition-all"
                                            >
                                                View My History
                                            </button>
                                        </div>
                            ...
                            {user?.role === 'Student' && session.status !== 'active' && session.status !== 'completed' && (
