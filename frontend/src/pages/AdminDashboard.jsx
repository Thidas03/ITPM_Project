                                ...
                                <div className="bg-gray-800 p-8 rounded-[2.5rem] border border-slate-100 shadow-xl">
                                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-[10px] font-black text-slate-400 uppercase">Cancelled</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-6 gap-4 h-64 items-end px-4 border-b border-gray-700 pb-2">
                                        {attendanceReport.trends.map((item, idx) => {
                                            const maxSessions = Math.max(...attendanceReport.trends.map(t => t.attended + t.cancelled)) || 1;
                                            return (
                                                <div key={idx} className="flex flex-col items-center gap-2 group h-full justify-end">
                                                    <div className="flex gap-1 items-end h-[80%] w-full max-w-[50px]">
                                                        <div 
                                                            ...
                                                            title={`Attended: ${item.attended}`}
                                                            style={{ height: `${(item.attended / maxSessions) * 100}%` }}
                                                        ></div>
                                                        <div 
                                                            className="flex-1 bg-gradient-to-t from-amber-600 to-amber-400 rounded-t-md transition-all duration-500 group-hover:opacity-80"
                                                            title={`Cancelled: ${item.cancelled}`}
                                                            style={{ height: `${(item.cancelled / maxSessions) * 100}%` }}
                                                        ></div>
                                                    </div>
                                                ...
                                                attendanceReport.misuseBehavior.map(u => (
                                                    <div key={u._id} className="flex items-center justify-between p-4 bg-gray-900 rounded-2xl border border-slate-100 hover:border-amber-400 transition">
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div>
                                                                <p className="font-bold text-gray-300 leading-none mb-1">{u.firstName} {u.lastName}</p>
                                                                <p className="text-[10px] text-slate-500 font-black tracking-tighter uppercase">High Cancellation Pattern</p>
                                                            </div>
                                                        </div>
                                                        <div className="flex gap-3">
                                                           {u.cancellations > 0 && (
                                                                <div className="text-center px-2 py-1 bg-amber-500/10 rounded-lg">
                                                                    <p className="text-xs font-black text-amber-500">{u.cancellations}</p>
                                                                    <p className="text-[8px] font-bold text-slate-400 uppercase">Cancelled</p>
                                                                </div>
                                                           )}
                                        ...
                                        <table className="w-full text-left border-collapse">
                                            <thead>
                                                <tr className="bg-gray-900 shadow-sm border-b border-gray-700">
                                                    ...
                                                    <th className="px-8 py-5 text-[10px] font-black text-slate-500 uppercase tracking-widest text-center">Cancelled</th>
                                            ...
                                            <tbody className="divide-y divide-gray-700/50">
                                                {attendanceReport.allStudentAttendance.map((student, idx) => (
                                                    <tr key={student._id || idx} className="group hover:bg-gray-700/30 transition-colors">
                                                        ...
                                                        <td className="px-8 py-6 text-center">
                                                            <span className="text-sm font-black text-amber-500">{student.cancellations || 0}</span>
                                                            <p className="text-[8px] font-bold text-slate-500 uppercase">Cancelled</p>
                                                        </td>
                                                        <td className="px-8 py-6">
